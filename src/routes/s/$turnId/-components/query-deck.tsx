 ...
 *    删除——那是**微调**，快过重写整句。它不必独自承担表达整个查询的责任：
 *    它表达不了的部分由左边那句话表达。这排 chip 就是读出来的全部：句子里没
 *    变成 chip 的字，就是没参与找人的字。
 *
 * 再往下的人数与图例（`result-list.tsx` 的 `ResultHeader`）回答的是「这份名单是
 * 什么」。它归名单，不归这里。
 *
 * **定高，而且吸在顶栏下沿。** 两件事是一起的：
 *
 ...
 *   唯一能改「问的是什么」的地方。
 * - 定高（`--deck-height`），因为左右两栏和名单卡片都按那个值吸顶、留出空间。
 *   所以句子长了就截断（整句挂在 `title` 上），而改写框和错误提示不放在横条
...
export function QueryDeck({
	...
	return (
		...
		<header className="sticky top-(--header-height) z-stick min-h-(--deck-height) bg-canvas/80 backdrop-blur-sm before:absolute before:inset-x-0 before:bottom-0 before:h-px before:bg-border/64 lg:h-(--deck-height)">
			{/*
			 * 高度挂在 `<header>` 上，不挂在这一行上：改写的时候这一行整个不渲染
			 * （被浮出来的框盖住的东西不该还能 Tab 到，那句话也不该在屏幕上出现
			 * 两遍），而横条的高度是下面三栏留出空间的依据，不能跟着一起消失。
			 *
			 * lg 以上是**一行**：句子先截断（`truncate`），chips 不让——读错的那一个
