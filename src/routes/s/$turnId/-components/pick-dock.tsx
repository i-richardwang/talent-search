...
/**
 * 挑上人之后浮起来的那一小块：挑了几个、清空、导出。
 *
 * **一个人都没挑的时候它不在。** 挑人是个有开始有结束的活，中间那段才需要一个
 * 收口；空着的时候摆一条「已选 0 人 · 清空 · 导出」，是在屏幕上留一样不会变的
 * 东西（AGENTS.md），而那三个词此刻一个也按不动。
 *
 * 它浮在名单下沿而不是吸在名单顶上：顶上那一叠（顶栏、查询带、名单表头）已经
 * 三层了，再插一条就是第四条横杠，而这一条讲的根本不是「这份名单是什么」。
 * 浮下来还顺带解决了「挑到第三十个人时导出的入口在哪」——它一直在手边。
 *
