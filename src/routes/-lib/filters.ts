 ...
 * 一维的筛选组。
 *
 * 集合维一维之内可以选多项（它们之间是「或」）；单值维点别的直接换掉，再点
 * 选中的那一个是取消。这个差别只活在 `toggle` 这一处——左栏画出来的七维长得
 * 一模一样，而写回的形状由 `dimensions.ts` 说了算。
 *
...
function field(key: DimKey, facets: Facets, view: View): FilterField {
	...
	return {
		...
		toggle: (v) => {
			if (!isMulti(key)) return write(key, values.includes(v) ? [] : [v], all);
			const next = new Set(values);
			if (!next.delete(v)) next.add(v);
			return write(key, [...next], all);
		},
	};
}
