...
/**
 * 在同一代语料上完成一组读取，并告诉回调它站在哪一代上。
 *
 * `embedding_space` 是换代门闩：ETL 的发布事务先独占锁它，所有跨语句读取先共享
 * 锁它，因此换代只能发生在整组读取之前或之后，一次结果不会混进两代数据。
 *
 * **模型调用不许发生在门闩内。** 嵌入和重排是秒级的，而 ACCESS EXCLUSIVE 在
 * Postgres 里是排队的：一个慢端点挡住一个等着发布的 ETL，那个 ETL 就挡住排在
 * 它后面的每一个新读者——一次端点抖动会变成全站不可用。所以准入（召回 + 重排）
 * 在门闩外算完，再进门闩取数。
 *
 * 代价是算出来的计划可能过期，`generation` 就是给调用方核对这件事用的：它是
 * `embedding_space` 那一行的行版本号，而整库重灌每次都会重写这一行
 * （`etl/load.py` 的 `_publish_corpus`），所以行版本变了就是换了一代。不另立一列
...
export function withCorpusSnapshot<T>(
	...
	return db.transaction(async (store) => {
		...
		const [row] = rows.rows;
		if (!row) throw new Error("语料没有嵌入空间元数据，请先运行 ETL");
