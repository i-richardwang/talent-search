 ...
 * 有受控证据（任职记录）才算命中，只在简历里提过的人一律不算。所以这里钉三件
...
const render = (view: View, strongOn: number, terms = TERMS) =>
	visibleText(
...
describe("只看任职记录可查的", () => {
	...
	test("关着的时候带着人数：点下去还剩几个，不点就知道", () => {
		...
		assert.match(render({}, 7), /只看任职记录可查的\s*7/);
	});
...
describe("不给死路", () => {
	test("一个人都数不出来时这个开关不出现", () => {
		...
		// 只有这一档是布尔的，没有行可以禁用。
		assert.ok(!render({}, 0).includes("只看任职记录可查的"));
	});

	test("已经打开的永远留着，否则没有任何东西能关掉它", () => {
		assert.ok(render({ strong: true }, 0).includes("只看任职记录可查的"));
	});
});
