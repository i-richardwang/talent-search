 ...
 * 用户只会拿到一句不对症的建议。所以每一支都在这里钉死；说什么、给哪条出路
...
describe("没有可执行的语义要求", () => {
	test("条件全被停用——和「没有这样的人」正好相反", () => {
		assert.deepEqual(why({ evidence: "~渠道运营,~+带团队" }), {
			kind: "allDisabled",
		});
	});

	test("停用的排除词不算「我把条件停了」：它本来就不产出人", () => {
		assert.deepEqual(why({ evidence: "~-实习" }), { kind: "excludeOnly" });
	});
	...
	test("一个词都没解析出来", () => {
		assert.deepEqual(why({ evidence: "" }), { kind: "noConditions" });
	});
...
describe("有要求但没人", () => {
	...
	test("开着证据要求但关掉也没人时，成因不是它", () => {
		assert.deepEqual(
			why({ terms, evidence: "算法", filters: { strong: true } }),
			{ kind: "unmet" },
		);
	});
	...
	test("筛选太窄", () => {
		assert.deepEqual(
			why({ terms, evidence: "算法", filters: { level: ["P7"] } }),
			{ kind: "filtered" },
		);
	});
	...
	test("什么都没筛还是空：AND 没满足", () => {
		assert.deepEqual(why({ terms, evidence: "算法" }), { kind: "unmet" });
	});
});
