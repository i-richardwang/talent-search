...
for (const r of top) {
	...
	for (const h of r.hits) {
		...
		console.log(
			`     ${h.term}${h.value === h.term ? "" : ` ≈${h.value}`} ←[${h.route}${h.phrase ? `「${dots(h.involvement, h.phrase)}」` : ""} ${Math.round(h.relevance * 100)}%] ${period(h.startDate, h.endDate)} ${h.org} ${h.title}${h.seq ? ` (${h.seq})` : ""}`,
		);
	}
}
await pool.end();
