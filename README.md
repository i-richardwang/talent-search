...
- **整理**（后台，每天一次）：把能力词的不同写法归并成标准词，一个词一周判一次；决定写进 `skill_alias`，能力词那一路的边随之改指标准词。判卷这一步可以交给外部 agent，见下。
...
整理一轮分四步：**作废、结算、出题、判卷**。前三步是机械的——按向量把相似的能力词圈成组、把答卷落进对照表；只有判卷需要判断力（「销售团队管理」算不算「团队管理」，「团队培训」算不算）。这一步可以换人做：
  ...
  "questions": [
    {
      "id": 418,
      "head": "团队管理",                       // 标准词，不由裁判选
      "candidates": [                           // 逐个判断该不该并进标准词
        { "word": "人员管理", "people": 12 },
        { "word": "团队培训", "people": 5 }
      ],
      "askedAt": "2026-09-10T03:00:12Z",
      "expiresAt": "2026-09-17T03:00:12Z"       // 到点没人答就作废，那个词下一轮重出
    }
  ],
...
curl -X POST -H "Authorization: Bearer $REVIEW_TOKEN" -H 'content-type: application/json' \
  -d '{"id":418,"judge":"hr-bot","judgments":[
        {"word":"人员管理","why":"同一件事的不同写法","alias":true},
        {"word":"团队培训","why":"只是其中一个环节","alias":false}]}' \
  "$HOST/api/review"
```

`judge` 是裁判自报的名字（1 到 40 个字母、数字、`-`、`_` 或 `.`），只作显示——管理页 `/skills` 上每条决定都写着是谁判的。`why` 收下但不读，它在那里是为了约束判断：让裁判先说理由再下结论，比让它直接列名单准。回应是 `200`（收下，`{"accepted":true}`）、`400`（形状不对）、`401`（凭据不对）、`404`（接口没开，或这道题不在队列里）、`409`（已经有人答过）。

交卷只写题那一行。对照表和边由整理任务在写者锁里改，所以交卷不必等派生放锁；交完会催一次整理，几分钟内生效，催不动也不丢，下一轮照样结算。什么时候生效是服务端自己的事，回应里不说。答卷要过和自带模型同一处收窄：不在候选里的词、`alias` 不是 `true` 的判断，一律当没说。
...
| `EXTRACT_BASE_URL` | 否 | OpenAI 兼容的聊天端点（`/chat/completions`）；派生用它从入职前简历描述抽能力词和做过的事，把入职前岗位对到公司序列，整理用它归并能力词的写法。岗位名、公司名和描述原文会送到这里 |
...
真实员工数据不进入版本库。同步从配置的数据源读取，完成校验与切段后写入 Postgres；派生把每段经历的四路原文（序列、岗位、部门或公司、简历描述）去重成「说法」送到嵌入端点换成向量；配了抽取端点时，入职前经历的简历描述还会送去读成能力词和做过的事，作为另外两路说法一并嵌入；每段入职前经历的岗位名、公司名和描述也会送去对到公司序列树，结果只供序列筛选读；整理能力词时，库里的能力词和人数也会送去让模型判断哪些是同一项能力的不同写法。查询时召回的说法会送到重排端点判定相关度。除了这三个端点，个人经历不出这台机器；端点选内网还是公网由部署方决定。仓库里的样例和测试夹具全部是合成数据。

判卷交给外部 agent 时（`REVIEW_JUDGE=external`），出这台机器的只有能力词和写这个词的人数——没有姓名、工号，也没有简历原文。这条接口的数据边界比抽取端点窄得多，和查询理解同一档。
...
src/corpus/aliases.ts        能力词对照表：整理任务圈组、问模型、合并写法、边改指标准词
