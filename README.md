...
- **查询是一条记录，视图是几个 URL 参数。** 一次「我要找什么人」落成 `search_turn`
  ...
  留在 query string 里。前者值得留存、可重新理解、分享出去问的仍是同一个问题，
...
- 查询由若干条**要求**组成：要求之间 AND；一条要求可以有多个**说法**（用户并列的
  「A 或 B」，以及查询理解补的相近说法），说法之间 OR，相近说法按降档计分。
- 必须要求采用 AND；加分要求只抬升排名；排除词**否决经历段**——命中它的段不再
  作为任何要求的证据，人只有在失去全部证据时才出局；停用词保留配置但不参与当前检索。
