...
为什么拒绝、合并了哪些写法、各表最后几行；失败时还有那一步报的错连同它的来由。

「此刻有没有人在跑」问的是那把咨询锁，不是表里那一列：持锁的连接一断，这件事当场就不成立，
所以进程中途没了留下的那一行读作「中断」而不是一次永远跑不完的导入。

导入先把整份语料准备在数据库连接私有的暂存表里，再用一个短事务原子发布。准备失败不影响正在服务的语料；
