...
- **整理**（后台，每天一次）：出两种题给裁判。**圈组**：给能力词建词表，一个词一周判一次，决定写进 `skill_term`。词表只决定筛选栏怎么摆，不改人身上的词（边上永远是简历原话）。它分两层：**同一件事的不同写法**在筛选栏里合成一个标准词；**一个词属于哪个更宽的词**（「销售数据分析」属于「数据分析」）记一条归属，筛选栏沿归属往上聚——点「数据分析」看到做过各种数据分析的人，点「销售数据分析」只看到销售那一种。**释义**：给技能、做过的事、岗位名、序列名这些短说法各写一句它指什么，写进 `phrase_gloss`，检索判定时读的是「说法：释义」（为什么见下一节）。释义写一次永远有效，只有新来的说法才出题。判卷这一步可以交给外部 agent，见下。
...
整理一轮分四步：**作废、结算、出题、判卷**。前三步是机械的——按向量把相似的词圈成组、把答卷落进表；只有判卷需要判断力。题有两种：**圈组**问「人员管理」和「团队管理」是不是同一件事、「销售团队管理」属不属于「团队管理」；**释义**请裁判给一组相近的短说法各写一句它指什么（「客户开发」是销售拓展客户，「服务端开发」是写服务器程序），检索判定读的就是这一句。这一步可以换人做：
...
```jsonc
{
  "questions": [
    ...
    {
      "id": 419,
      "kind": "gloss",                          // 释义题：一批相近的短说法，最多 40 个
      "words": [
...
```bash
# 释义题的答卷：每个说法一句话，不超过 30 字，不重复说法本身
curl -X POST -H "Authorization: Bearer $REVIEW_TOKEN" -H 'content-type: application/json' \
...
`judge` 是裁判自报的名字（1 到 40 个字母、数字、`-`、`_` 或 `.`），只作显示——管理页 `/skills` 上每条决定都写着是谁判的。圈组题里 `why` 收下但不读，它在那里是为了约束判断：让裁判先说理由再下结论，比让它直接列名单准。`sameAs` 连成的每一片是同一件事，片里人最多的写法做标准词，裁判不选写法；`parent` 是裁判起的名字，可以是这组里没有、语料里也没人写过的词，写最近的一层。释义题里 `gloss` 空的、和说法一样的、超过 60 字的当没答，那个说法下一轮重出。回应是 `200`（收下，`{"accepted":true}`）、`400`（形状不对）、`401`（凭据不对）、`404`（接口没开，或这道题不在队列里）、`409`（已经有人答过）。

交卷只写题那一行。词表、释义和边由整理任务在写者锁里改，所以交卷不必等派生放锁；交完会催一次整理，几分钟内生效，催不动也不丢，下一轮照样结算。什么时候生效是服务端自己的事，回应里不说。答卷要过和自带模型同一处收窄：不在题里的词、指向题外的 `sameAs`、含着这个词的 `parent`（方向反了），一律当没说。
...
判卷交给外部 agent 时（`REVIEW_JUDGE=external`），出这台机器的只有短说法（能力词、做过的事、岗位名、序列名）和它下面的人数——没有姓名、工号，也没有简历原文。这条接口的数据边界比抽取端点窄得多，和查询理解同一档。
...
src/corpus/review.ts         整理任务：一轮的顺序（作废、结算、出题、判卷）
src/corpus/questions.ts      整理的题目队列：出题、拉题、交卷、作废；裁判是谁
src/corpus/vocabulary.ts     圈组题：词表的读写、出题与结算、模型调用
src/corpus/vocabulary-rules.ts 能力词的纯规则：圈组、收窄、标准词与归属，生产和验收共用
src/corpus/gloss.ts          释义题：哪些说法要释义、凑批出题、收窄与结算
...
- 条件之间 AND、同一个人；一条主张里的各项是同一段经历。一项可以有多个**取值**，
  ...
  只写代表取值，其余收在菜单里可逐项去掉；靠非代表词命中时证据行标「≈ 那个词」。
...
- 名次有两把尺，不相乘。**可信度**看证据来自哪一档：登记的序列或岗位、登记的部门或公司、
  ...
  加分的往上抬。默认先按可信度分档、档内按深度；表头上可以切成只看深度。
